...
def edit_text(text: str, max_chars: int = 4000, model: Optional[str] = None) -> Optional[str]:
    ...
    cfg = load_config()
    system = (
        "Ты — редактор новостной ленты. Приведи текст к единому, чистому "
        "виду, сохранив все факты, имена, числа, цитаты и смысл дословно.\n"
        "Правила обработки:\n"
        "- Убери рекламу и призывы подписаться на канал/бота (например «Подписывайся», "
        "«Больше новостей на…», «@канал», «Не забудь поставить лайк» и т.п.).\n"
        "- Убери повторяющиеся «хвосты»-подписи (например «// обратная связь», "
        "«@serbia», «🟦 Подписаться на канал»).\n"
        "- Убери декоративные эмодзи/смайлики, не несущие смысла (💬🔗☮️🩷 и подобные), "
        "но оставь смысловые (например валюты, флаги стран при упоминании).\n"
        "- Исправь пропущенные пробелы, убере лишние/пустые строки и прочий «шум».\n"
        "- Не добавляй от себя никаких пояснений, заголовков и водяных знаков.\n"
        "- Верни ТОЛЬКО итоговый отредактированный текст поста, без обёрток и markdown."
    )
    content = _chat(
        [
